...
등교 준비 잠금 모드는 평일 아침에 아이를 벌주거나 기능별로 막는 기능이 아니라, 앱 입장 단계에서 “등교 준비가 먼저”라는 가족 규칙을 안내하는 기능이다.

구현 기준:

- 기본 설정은 사용함, 월~금 06:30~08:30이다.
- 스플래시 후 아이 선택/PIN 전에 `isSchoolMorningLockActive()`로 판단한다.
- 잠금 중이면 `renderSchoolMorningLock()`을 표시하고, 아니면 기존 `renderKidGate()`로 간다.
- 잠금 화면에는 아이용 큰 버튼, 체크박스, 계속하기 버튼을 두지 않는다.
- 하단의 작은 `부모 메뉴`만 제공하고 매번 `0903`을 요구한다.
- 부모 인증 후 오늘만 잠금 해제, 사용/해제, 시작/종료 시간 변경을 할 수 있다.
- 기존 부모 메뉴 설정 탭에도 `등교 준비 잠금` 설정이 있다.
- 저장 구조는 `state.gameMeta.schoolMorningLock`이며 localStorage 키는 계속 `kidsPointGame_v1`이다.
- 기존 XP, 코인, PIN, 보관함, 승인 대기, 가족 보상, 두뇌 던전 데이터는 초기화하면 안 된다.
- `sw.js` 캐시는 `kids-mission-battle-v1.4.2c`이다.

다음 작업자는 패드 업로드 후 일반 시간과 잠금 시간 모두 실사용 검수를 해야 한다. 현재 시간이 잠금 시간이 아니면 부모 설정에서 시작/종료 시간을 임시로 현재 시간이 포함되게 바꿔 확인한다.
...
4. **패드 실사용 검수** — `TEST_CHECKLIST.md` 19절 v1.4-2c 항목 확인
